...
def main():
    ...
    df_real_train = df[df['intervention_name'].isin(intervention_list)]
    df_real_general = df[~df['studyid'].isin(df_real_train['studyid'])]

    # Downsample label 1 in df_real_general to match label 0
    label_1 = df_real_general[df_real_general['label'] == 1]
    label_0 = df_real_general[df_real_general['label'] == 0]
    label_1_downsampled = label_1.sample(n=label_0.shape[0], random_state=42)
    df_real_general = pd.concat([label_1_downsampled, label_0])
    df_real_general = df_real_general.sample(frac=1, random_state=42).reset_index(drop=True)

    print("Class distribution after downsampling in df_real_general:")
    print(df_real_general['label'].value_counts())
    ...
    real_texts, real_labels = load_examples(df_real_train)
    general_texts, general_labels = load_examples(df_real_general)

    real_train_dataset = Dataset.from_dict({'text': real_texts, 'label': real_labels})
    ...
    # Tokenize datasets
    tokenized_train_dataset_synth = synthetic_dataset.map(tokenize_function, batched=True)
    tokenized_train_dataset_real = real_train_dataset.map(tokenize_function, batched=True)

    # Prepare training datasets
    train_datasets = {
        'hybrid': concatenate_datasets([real_train_dataset, synthetic_dataset]),
    ...
    tokenized_train_datasets = {
        ...
        'real': tokenized_train_dataset_real,
        'synthetic': tokenized_train_dataset_synth
    }
    ...
    for seed in seed_values:
        ...
        for train_name, train_dataset in tokenized_train_datasets.items():
            ...
            training_args = TrainingArguments(
                output_dir=f'./results_{train_name}_{seed}_general',
                eval_strategy="epoch",
                per_device_train_batch_size=8,
                per_device_eval_batch_size=8,
                num_train_epochs=7,
                weight_decay=0.01,
                logging_dir=f'./logs_{train_name}_{seed}_general',
                logging_steps=10,
                save_strategy="epoch",
    ...
    for train_name in ['hybrid', 'real', 'synthetic']:
        print(f"\nFinal Results for {train_name.capitalize()} Dataset:")
